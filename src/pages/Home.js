...
const Home = () => {
  ...
  function getPromptString (prompt, cursor = true){
    return  (<div>
        <div><span className="color-prompt-white">╭─</span><span className="color-prompt-green">burakssen@burakssen.github.io</span> <span className="color-prompt-purple">~</span></div>
        <div><span className="color-prompt-white box">╰─$</span> {prompt}{cursor && <span className="cursor"></span>}</div>
    </div>);
  }
  ...
  const applyPrompt = (prompt) => {
    switch(prompt[0]){
      ...
      case "":
        ...
        break;
      default:
        setConversationList([...conversationList, {prompt: getPromptString(prompt[0], false), response: <div style={{color: "red"}}>Command <b>{prompt[0]}</b> not found</div>}]);
        break;
    }
  }
  ...
  return (
    <div className="App color-prompt-white">
      ...
      <form onSubmit={onFormSubmit}>
        ...
        <input type="text" autoFocus name="prompt" id="prompt" style={{opacity: 0}} autoComplete="off" onChange={(e) => setPrompt(e.target.value)}/>
      </form>
